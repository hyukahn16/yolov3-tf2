...
def YoloLoss(anchors, classes=80, ignore_thresh=0.5):
    def yolo_loss(y_true, y_pred):
        ...
        # print(pred_class[:,:,:,:,])
        # print(tf.reduce_max(pred_obj))
        # print(tf.math.reduce_max(pred_obj))
        ...
        # print(tf.math.reduce_max(true_obj))
        # tf.print(tf.math.reduce_max(true_wh), summarize=-1)
        ...
        obj_mask = tf.squeeze(true_obj, -1) # all zeros when labels zeros
        # 4.1 detection suppresion
        # obj_mask = tf.ones(obj_mask.shape)
        inv_obj_mask = 1 - obj_mask
        ...
        xy_loss = box_loss_scale * \
            tf.reduce_sum(tf.square(true_xy - pred_xy), axis=-1)
            # obj_mask * box_loss_scale * \
            # tf.reduce_sum(tf.square(true_xy - pred_xy), axis=-1)
        wh_loss = box_loss_scale * \
        ...
        # obj_loss *= 10
        # class_loss /= 1000
        # print(str(xy_loss) + str(wh_loss) + str(obj_loss) + str(class_loss))

        return tf.reduce_sum(pred_obj)
